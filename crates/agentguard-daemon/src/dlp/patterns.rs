...
///
/// Formato: `(nombre_del_patrón, regex_sin_compilar)`. El nombre es el
/// único campo que llega a los logs/incidentes — los bytes que hicieron
/// match nunca se persisten.
...
/// Compila `DEFAULT_PATTERNS`. Error solo si hay un bug en las regex del
/// código; por eso el test `default_patterns_all_compile` actúa de guard.
...
/// Compila una lista de patrones custom (del `config.toml`).
...
/// Compila defaults + custom. El primer match (en orden) gana y corto-
/// circuita el escaneo. Los custom van al final para que overrides de
/// defaults funcionen si se definen con el mismo nombre.
pub fn compile_all(
    custom: &[(String, String)],
) -> Result<Vec<CompiledPattern>, PatternError> {
...
/// **nunca** el contenido matcheado (política de seguridad).
...
mod tests {
    ...
    fn default_patterns_all_compile() {
        // Cualquier regex con sintaxis mala en DEFAULT_PATTERNS rompería
        // el deploy — este test actúa de sanity check.
    ...
    fn first_match_short_circuits_on_first() {
        // Dos patrones que ambos matchean el mismo input: el primero gana.
        let custom = vec![
