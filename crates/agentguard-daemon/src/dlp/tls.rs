//! HTTPS MITM: emisión de certificados leaf firmados por la CA local.
//!
//! Arquitectura:
//! - `LeafIssuer` mantiene la CA en memoria (cert + key) y un **cache**
//!   de `rustls::ServerConfig` por hostname. Emitir y firmar un leaf es
//!   caro (~ms en ECDSA), así que la primera vez que un agente habla con
//!   `api.openai.com` pagamos la firma y luego cacheamos.
//! - Cada ServerConfig firma con el mismo CA, pero con un CN diferente —
//!   cualquier cliente que haya confiado la CA local vía `root.crt` ve un
//!   certificado válido. Cualquier cliente que hace pinning (la CLI de
//!   Anthropic, por ejemplo) rechazará el cert: este es el behavior
//!   esperado y documentado.
//! - Los leaf certs tienen vida corta (30 días). Si el daemon se queda
//!   corriendo más tiempo, regenera automáticamente al expirar.
//!
//! **Seguridad:** la clave privada de la CA nunca sale de memoria del
//! daemon. Los leaf certs vivían solo en RAM; no los persistimos.
...
/// Tope del cache de leaf certs. Más allá de esto evictamos LRU-ish
/// (por timestamp de creación). En la práctica un usuario típico habla
/// con <50 hosts distintos durante una sesión.
...
/// Emisor de certificados leaf firmados por la CA local.
///
/// Barato de clonar — todos los clones comparten el mismo cache
/// (`Arc<Mutex<HashMap>>` interno).
#[derive(Clone)]
...
impl LeafIssuer {
    /// Crea el issuer a partir de la CA cargada.
    ...
    /// Obtiene (o emite) el `ServerConfig` para `host`. Garantizado
    /// O(1) amortizado una vez que el hostname está cacheado.
    pub fn server_config_for(&self, host: &str) -> Result<Arc<ServerConfig>, TlsError> {
        // Fast path: hit de cache.
        if let Some(cfg) = self.cache_get(host) {
            return Ok(cfg);
        }
        // Slow path: emitir y cachear.
    ...
    fn cache_get(&self, host: &str) -> Option<Arc<ServerConfig>> {
        ...
        if now.saturating_sub(entry.created_at) > max_age.saturating_sub(3_600) {
            // El leaf está cerca de expirar; fuerza regeneración.
            return None;
        }
    ...
    fn issue_and_cache(&self, host: &str) -> Result<Arc<ServerConfig>, TlsError> {
        ...
        // Generar keypair para el leaf.
        let leaf_key = KeyPair::generate().map_err(|source| TlsError::LeafBuild {
            host: host.to_string(),
            source,
        })?;

        // Preparar los params del leaf: SAN = [host], CN = host, validez corta.
        ...
        // Insertar en cache (con eviction naïve si pasamos el límite).
    ...
    /// Tamaño actual del cache (útil para tests/metrics).
...
/// Validación conservadora de nombre DNS. No aceptamos IPs aquí — los
/// clientes que hacen SNI con una IP son raros y abrir esa puerta
/// complicaría el SAN del cert.
...
mod tests {
    ...
    fn make_ca() -> LocalCa {
        ...
        // LocalCa retiene su dir internamente; TempDir se limpia al caer.
        // Para tests no nos importa — TempDir vive hasta el final del test.
        let ca = LocalCa::generate_and_persist(tmp.path().join("ca")).expect("ca");
        // Forzar que el TempDir sobreviva todo el test.
        std::mem::forget(tmp);
        ca
    }
    ...
    fn issues_cert_for_hostname_and_caches_it() {
        ...
        // Segunda llamada al mismo host → mismo Arc (cache hit).
