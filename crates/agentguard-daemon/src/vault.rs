...
impl Vault {
    ...
    ///
    /// Caller debe haber pausado la protección kernel antes de llamar
    /// (o el LSM bloqueará las escrituras en las zonas protegidas).
...
/// Descubrimiento recursivo de archivos. Sigue `read_dir` estándar — los
/// symlinks se incluyen como entradas (no se siguen) para evitar ciclos.
async fn collect_files(root: &Path) -> Result<Vec<PathBuf>, VaultError> {
    ...
    if !metadata.is_dir() {
        // Symlinks a dir, fifos, sockets... no los respaldamos.
        return Ok(files);
    }
...
fn get_permissions(path: &Path) -> u32 {
    #[cfg(unix)]
    {
        ...
        // unwrap-ok: tolerate read error by falling back to a sane default.
    }
    #[cfg(not(unix))]
    {
        let _ = path;
...
mod tests {
    ...
    async fn dedup_identical_content() {
        ...
        // Solo debe haber 2 blobs únicos en el directorio del snapshot
    ...
    async fn list_returns_newest_first() {
        ...
        // Asegurar que el siguiente snapshot tenga timestamp >= al primero
    ...
    async fn cleanup_removes_only_old_snapshots() {
        ...
        // keep_days = 30 → no debería borrar nada
        let removed = vault.cleanup(30).await.unwrap();
        assert_eq!(removed, 0);
        assert!(vault.root().join(&snap.id).exists());

        // keep_days = 0 → todo lo que tenga >0s de edad se va
        tokio::time::sleep(std::time::Duration::from_millis(1100)).await;
    ...
    async fn restore_preserves_unix_permissions() {
        ...
        // Cambiar permisos "maliciosamente" y borrar
