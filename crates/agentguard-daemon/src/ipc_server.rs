    ...
    fn execute(&self, cmd: IpcCommand) -> IpcResponse {
        match cmd {
            ...
            IpcCommand::Protect { path, .. } => {
                IpcResponse::Ok {
                    message: format!(
                        "path {path} registered (restart daemon to apply)"
                    ),
                }
            }
            ...
            IpcCommand::Unprotect { path } => {
                IpcResponse::Ok {
                    message: format!(
                        "path {path} removed (restart daemon to apply)"
                    ),
                }
            }
            ...
            IpcCommand::SnapshotRestore { id, yes } => {
                ...
                match self
                    .runtime
                    .block_on(self.vault.restore(&id))
                {
                    Ok(()) => IpcResponse::Ok {
            ...
            IpcCommand::SnapshotCleanup { keep_days } => {
                match self
                    .runtime
                    .block_on(self.vault.cleanup(keep_days))
                {
            ...
            IpcCommand::Incidents { .. } => IpcResponse::Incidents {
                lines: vec![
                    "incidents log not yet implemented (Fase 2.7+)".into(),
                ],
            },
...
mod tests {
    ...
    fn ping_returns_pong() {
        ...
        let server = IpcServer::new(vault, config, "test-backend", "userspace");
    ...
    fn status_returns_config_info() {
        ...
        let config = config::Config::default().resolve().expect("config");
        let vault = Vault::with_dir(tmp.path().join("vault")).expect("vault");
        let server = IpcServer::new(vault, config, "ebpf-lsm", "kernel-level");
    ...
    fn snapshot_restore_without_yes_rejects() {
        ...
        let config = config::Config::default().resolve().expect("config");
        let vault = Vault::with_dir(tmp.path().join("vault")).expect("vault");
        let server = IpcServer::new(vault, config, "test", "userspace");
