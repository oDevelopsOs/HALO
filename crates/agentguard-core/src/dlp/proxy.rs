...
const MAX_BODY_SCAN_BYTES: u64 = 2 * 1024 * 1024;
            ...
            loop {
                tokio::select! {
                    ...
                    accept = listener.accept() => {
                        ...
                        let is_connect = peek.starts_with(b"CONNECT") && tls_issuer.is_some();

                        if is_connect {
                            let text = String::from_utf8_lossy(&peek);
                            let parts: Vec<&str> = text.split_whitespace().collect();
                            let (host, port) = if parts.len() >= 2 && parts[1].contains(':') {
                                let a = parts[1];
                                let sep = a.find(':').unwrap_or(0);
                                (a[..sep].to_string(), a[sep + 1..].parse::<u16>().unwrap_or(443))
                            } else {
                                (String::new(), 443)
                            };
                            ...
                            if !host.is_empty() {
                                ...
                                let Some(issuer) = tls_issuer.as_ref().cloned() else { continue; };
                        ...
                        let prepend = PrependBuf { buf, pos: 0, inner: stream };
...
async fn handle_request(
    ...
    let body_bytes = match read_body_bounded(body, MAX_BODY_SCAN_BYTES).await {
        ...
        Err(BodyReadError::TooLarge) => {
            tracing::warn!(destination = %uri_for_log, limit = MAX_BODY_SCAN_BYTES, "request body exceeds DLP scan limit — forwarded without scanning");
            Bytes::new()
        }
    ...
    if let Some(matched) = first_match(&patterns, &haystack) {
        ...
        match action {
            DlpAction::Block => {
                return text_response(StatusCode::FORBIDDEN, &format!(
...
mod tests {
    ...
    async fn setup_mitm_e2e(action: DlpAction) -> (DlpProxyHandle, SocketAddr, oneshot::Sender<()>) {
        ...
        let mut server_params = CertificateParams::new(vec!["127.0.0.1".into()]).expect("san");
            ...
            loop {
                tokio::select! {
                    ...
                    accept = tcp_listener.accept() => {
                        let (stream, _) = match accept { Ok(v) => v, Err(_) => break };
                        let acc = acceptor.clone();
                        tokio::spawn(async move {
                            let tls = match acc.accept(stream).await { Ok(t) => t, Err(_) => return };
    ...
    async fn mitm_client_send(proxy_addr: SocketAddr, server_addr: SocketAddr, ca_pem: &str, payload: &[u8]) -> Vec<u8> {
        ...
        assert!(resp.contains("200"), "CONNECT response was not 200: {resp}");
    ...
    async fn mitm_e2e_block_drops_tls_connection() {
        ...
        let (handle, server_addr, server_shutdown) = setup_mitm_e2e(DlpAction::Block).await;
